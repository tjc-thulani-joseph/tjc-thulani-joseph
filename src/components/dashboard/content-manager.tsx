...
type UrlErrors = Record<string, string>;
...
function validateUrls(schema: ContentSchema, draft: Draft): UrlErrors {
...
export function ContentManager({ schema, description }: { schema: ContentSchema; description: string }) {
  ...
  const save = useMutation({
    mutationFn: async ({ draft, record, publish }: { draft: Draft; record: ContentRecord | null; publish: boolean }) => {
      const errors = validateUrls(schema, draft);
    ...
    onSuccess: (_data, variables) => {
      toast.success(variables.publish ? `${schema.singular} published` : `${schema.singular} saved as draft`);
      setEditing(null);
      setUrlErrors({});
      invalidate();
    },
  ...
  function openNew() {
    setUrlErrors({});
    setEditing({ record: null, draft: emptyDraft(schema) });
  }

  function openEdit(record: ContentRecord) {
    setUrlErrors({});
    setEditing({ record, draft: draftFromRecord(schema, record) });
  }

  function setField(key: string, value: unknown) {
    if (urlErrors[key]) setUrlErrors((current) => {
      const next = { ...current };
      delete next[key];
      return next;
    });
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {list.isPending ? (
        <div className="mt-6 space-y-3"><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /></div>
      ) : list.data?.error ? (
        <p className="mt-6 text-sm text-muted-foreground">{list.data.error.message}</p>
      ) : items.length === 0 ? (
        <p className="mt-6 text-sm text-muted-foreground">Nothing here yet. Create a {schema.singular.toLowerCase()} and publish it to see it on the public site.</p>
      ) : (
            ...
            return (
              <Card key={item.id} className="surface-panel overflow-hidden border-border">
                ...
                <CardContent className="space-y-3 p-5">
                  <div className="flex items-start justify-between gap-3"><h2 className="font-display text-base font-semibold">{item.title ?? "Untitled"}</h2><span className="shrink-0 text-[10px] uppercase tracking-[0.18em] text-gold/80">{item.status}</span></div>
                  {item.category && <p className="text-xs text-muted-foreground">{item.category}</p>}
                  <div className="flex flex-wrap gap-2 pt-1">
                    <Button size="sm" variant="outline" className="hairline-gold bg-transparent" onClick={() => openEdit(item)}><Pencil className="size-4" aria-hidden /> Edit</Button>
                    <Button size="sm" variant="ghost" disabled={togglePublish.isPending} onClick={() => togglePublish.mutate(item)}>{published ? <EyeOff className="size-4" aria-hidden /> : <Eye className="size-4" aria-hidden />}{published ? "Unpublish" : "Publish"}</Button>
                    <Button size="sm" variant="ghost" onClick={() => setDeleting(item)}><Trash2 className="size-4" aria-hidden /></Button>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      <Dialog open={Boolean(editing)} onOpenChange={(open) => !open && setEditing(null)}>
        <DialogContent className="max-h-[88vh] max-w-2xl overflow-y-auto">
          <DialogHeader><DialogTitle>{editing?.record ? `Edit ${schema.singular.toLowerCase()}` : `New ${schema.singular.toLowerCase()}`}</DialogTitle></DialogHeader>
          {editing && <div className="space-y-5 py-2">{schema.fields.map((field) => renderField(field, editing.draft))}</div>}
          <DialogFooter className="gap-2">
            <Button variant="ghost" onClick={() => setEditing(null)}>Cancel</Button>
            <Button variant="outline" className="hairline-gold bg-transparent" disabled={save.isPending} onClick={() => editing && save.mutate({ draft: editing.draft, record: editing.record, publish: false })}>Save draft</Button>
            <Button disabled={save.isPending} onClick={() => editing && save.mutate({ draft: editing.draft, record: editing.record, publish: true })}>Publish</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={Boolean(deleting)} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete {deleting?.title ?? "this record"}?</AlertDialogTitle><AlertDialogDescription>It is removed from the public site immediately. The stored file stays in your Media Library.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => { if (deleting) destroy.mutate(deleting); setDeleting(null); }}>Delete</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
